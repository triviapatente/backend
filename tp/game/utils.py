...
def getOpponentFrom(game):
    partecipation = Partecipation.query.filter(Partecipation.game_id == game.id).filter(Partecipation.user_id != g.user.id).first()
    return User.get(partecipation.user_id)
