...
def getRecentGames(user):
    ...
    active_recent_games = db.session.query(Game).join(Partecipation).filter(Partecipation.user_id == user.id).with_entities(Game, my_turn, remainingAnswersCount, myScore, opponentScore).filter(Game.ended == False).order_by(desc("my_turn"), Game.updatedAt.desc()).all()
    ...
    if len(active_recent_games) < RECENT_GAMES_PER_PAGE:
        ...
        ended_recent_games = db.session.query(Game).join(Partecipation).filter(Partecipation.user_id == user.id).with_entities(Game, my_turn, remainingAnswersCount, myScore, opponentScore).filter(Game.ended == True).order_by(Game.updatedAt.desc()).limit(limit).all()
