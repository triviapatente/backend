...
def getCategoryPercentages(user):
    ...
    output = Category.query.with_entities(Category.id, Category.hint, correct_answers, total_answers).join(Quiz, Quiz.category_id == Category.id).join(Question, Question.quiz_id == Quiz.id).filter(Question.user_id == g.user.id, Question.createdAt == max_created).order_by(Category.hint).group_by(Category.id, Category.hint).all()
